...
typedef struct SampleSieve_params {
  int32_t k;
  cache_obj_t **eviction_candidates;
} SampleSieve_params_t;

static const char *DEFAULT_CACHE_PARAMS = "k=5";

static void SampleSieve_free(cache_t *cache);
static bool SampleSieve_get(cache_t *cache, const request_t *req);
static cache_obj_t *SampleSieve_find(cache_t *cache, const request_t *req,
                                     bool update_cache);
static cache_obj_t *SampleSieve_insert(cache_t *cache, const request_t *req);
static cache_obj_t *SampleSieve_to_evict(cache_t *cache, const request_t *req);
static void SampleSieve_evict(cache_t *cache, const request_t *req);
static bool SampleSieve_remove(cache_t *cache, obj_id_t obj_id);
static void SampleSieve_parse_params(cache_t *cache,
...
cache_t *SampleSieve_init(const common_cache_params_t ccache_params,
  ...
  cache_t *cache = cache_struct_init("SampleSieve", ccache_params_copy,
                                     cache_specific_params);
  cache->cache_init = SampleSieve_init;
  cache->cache_free = SampleSieve_free;
  cache->get = SampleSieve_get;
  cache->find = SampleSieve_find;
  cache->insert = SampleSieve_insert;
  cache->to_evict = SampleSieve_to_evict;
  cache->evict = SampleSieve_evict;
  cache->remove = SampleSieve_remove;
  ...
  cache->eviction_params =
      (SampleSieve_params_t *)malloc(sizeof(SampleSieve_params_t));
  SampleSieve_params_t *params =
      (SampleSieve_params_t *)(cache->eviction_params);
  memset(params, 0, sizeof(SampleSieve_params_t));

  SampleSieve_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    SampleSieve_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "SampleSieve-%d",
           params->k);
...
static void SampleSieve_evict(cache_t *cache, const request_t *req) {
  cache_obj_t *obj_to_evict = SampleSieve_to_evict(cache, req);
